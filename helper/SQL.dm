...
DBConnection
	...
	proc/Connect(dbi_handler = dbi, user_handler = user, password_handler = password, cursor_handler)
		if(!src)
			return 0
	...
	proc/SelectDB(database_name, dbi)
		if(IsConnected())
			Disconnect()
		return Connect("[dbi?"[dbi]":"dbi:mysql:[database_name]:[sql_address]:[sql_port]"]", user, password)

	proc/NewQuery(sql_query, cursor_handler = default_cursor)
...
DBQuery
	...
	proc/Connect(DBConnection/connection_handler)
		db_connection = connection_handler

	proc/Execute(sql_query = sql, cursor_handler = default_cursor)
		Close()
	...
	proc/SetConversion(column ,conversion)
		if(istext(column))
...
DBColumn
	...
	proc/SqlTypeName(type_handler = sql_type)
		switch(type_handler)
